...
namespace game
{
	...
	void TileObject::SetupTile(int x, int y)
	{
		...
		std::string biomeSprite = Utility::GetRandomElementFromContainer(biome->sprites);
		std::string biomeSpritePath = Utility::spritePath(biomeSprite);
		biomeDisplay->SetSprite(biomeSpritePath);
		//biomeDisplay->SetSprite(AssetManager::textures[biomeSprite]);

		std::string indexName = " (" + std::to_string(x) + "," + std::to_string(y) + ")";
